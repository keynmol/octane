module Octane
	...
	class Network
		...
		def add_layer(num_neurons, type=:sigmoid)
		...
		def calculate_output_deltas(network_output, expected_output)
			...
			puts "Deltas: #{output_deltas}" if @verbose
		end
			...
			(0..@layers.size-1).to_a.reverse.each_cons(2){|visited_layer, previous_layer|
				...
				@layers[previous_layer].each_with_index{|neuron, index|
					unless neuron.disabled
						...
						puts "Neuron(#{neuron.type}) #{index} on layer #{previous_layer}. Derivative: #{neuron.derivative(neuron.last_squashed)}. Outgoing weights: #{outgoing_weights}. Deltas on lower level: #{@layers[visited_layer].map(&:delta)}" if @verbose
						...
						puts "Setting delta to #{neuron.delta}" if @verbose
						puts "---" if @verbose
					end
				}
			}
			...
			(0..@layers.size-1).to_a.reverse.each_cons(2){ |layer, previous_layer|
				...
				@layers[layer].each_with_index{ |neuron,index|
					unless neuron.disabled
						...
						neuron.input_weights=neuron.input_weights.zip(weight_changes+[bias_change]).map{|a,b| a - b-@weight_decay*b}
					end
				}
			}
		end
		...
		def train(dataset, options={})
			...
			options[:epochs].times do |epoch|
				...
				before_training_results <<options[:before_training][self,[example, expected_output]] if options[:before_training]
			...
			result[:test_error]=test_errors.map{|err| err/options[:test_set].length} if options[:test_set]
		...
		def test(dataset)
			...
			dataset.each{|example,output|
				result=forward_pass(example)
				test_error+=result.zip(output).map{|r, o| (r-o)**2}.reduce(:+)/2
			}
			test_error/dataset.length
		end
